...
def _render_milestones_section(info):
    with st.container(border=True):
        ...
        cols = st.columns(2)
        with cols[0]:
            info['cad_files'] = st.radio(IAT_MILESTONES[0], YES_NO, index=1, horizontal=True)
            info['process_spec'] = st.radio(IAT_MILESTONES[1], YES_NO, index=1, horizontal=True)
            with st.columns(2)[0]:
                info['nests'] = st.radio(IAT_MILESTONES[2], YES_NO, index=1, horizontal=True)
            with st.columns(2)[1]:
                info["qty_nests"] = st.text_input("How Many Nests?", placeholder="4")
            info['plc_programming_standard'] = st.radio(IAT_MILESTONES[3], YES_NO, index=1, horizontal=True)
            info["sow_ergonomic_spec"] = st.radio(IAT_MILESTONES[4], YES_NO, index=1, horizontal=True)
            with st.columns(2)[0]:
                info["layout"] = st.radio(IAT_MILESTONES[5], YES_NO, index=1, horizontal=True)
            with st.columns(2)[1]:
                info["dimensions"] = st.text_input("Dimensions", placeholder="4cm x 2.5cm")
        with cols[1]:
            info["product_manufacturing_sheet"] = st.radio(IAT_MILESTONES[6], YES_NO, index=1, horizontal=True)
            with st.columns(2)[0]:
            ...
            with st.columns(2)[0]:
                info["estimated_cycle_time"] = st.radio(IAT_MILESTONES[8], YES_NO, index=1, horizontal=True)
            with st.columns(2)[1]:
                info["cycle_time"] = st.text_input("Estimated Cycle Time", placeholder="7.5 sec")
            with st.columns(2)[0]:
                info["special_handling"] = st.radio(IAT_MILESTONES[9], YES_NO, index=1, horizontal=True)
            with st.columns(2)[1]:
                info["special_handling_info"] = st.text_input("Special Handling Info")
            info["customer_has_samples"] = st.radio(IAT_MILESTONES[10], YES_NO, index=1, horizontal=True)

def _render_station_features_section(info):
    with st.container(border=True):
        subtitle_h3("Station Features")
        cols = st.columns(2)
        with cols[0]:
            info["station_type"] = st.selectbox(r"*Type of Station or Service?", IAT_STATION_TYPES)
        with cols[1]:
            info["station_type_info"] = st.text_area("Station or Service Type details")
        cols = st.columns(2)
        with cols[0]:
            info["process_type"] = st.selectbox(r"*Type of Process?", IAT_PROCESS_TYPES)
        with cols[1]:
            info["process_type_info"] = st.text_area("Process Type information details")
        cols = st.columns(2)
        with cols[0]:
            info["uut_handle_mode"] = st.selectbox(r"*How will the Unit be Handled?", IAT_UNIT_HANDLE_MODES)
        with cols[1]:
            info["uut_handle_mode_info"] = st.text_area("More information")
        info["device_under_process"] = st.selectbox("Device under process?", IAT_DEVICES_UNDER_PROCESS)
        cols = st.columns(2)
        with cols[0]:
            info["design_required"] = st.radio("Design (Drawings) are required?", YES_NO, index=1, horizontal=True)
        with cols[1]:
            info["design_required_info"] = st.text_area("Details for Required Design")
        cols = st.columns(2)
        with cols[0]:
            info["certifications_required"] = st.radio("Certifications are required?", YES_NO, index=1, horizontal=True)
        with cols[1]:
            info["certifications_info"] = st.text_area("Details for Required Certifications")
...
def create_iat_specific_sections(info):
    ...
    _render_station_features_section(info)
