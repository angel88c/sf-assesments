...
from pages.utils.constants import (
    ...
    WELL_TYPES, SIZE_TYPES, OPTIONS, ICT_FILE_TYPES
)
from pages.utils.global_styles import subtitle_h3, subtitle_h4

def _render_feature_fixture_section(info):
    with st.container(border=True):
        subtitle_h3("Feature Fixture")
        info["inline_bottom_side"] = st.text_input('For InLine systems, which is the bottom side?', placeholder="Bottom side of PCB for Test System, e. g. Conenctor Side, main uc side, Layer")
        ...
        with col1:
            info['versions'] = st.number_input("How many versions?", placeholder="Enter the number of versions to consider", min_value=1)

def _render_flash_programming_section(info):
    with st.expander("Flash Programming", expanded=False):
        with st.container(border=True):
            info['flash_programming'] = st.radio('Flash Programming?', REQ_OPTIONS, horizontal=True)
            col1, col2 = st.columns(2)
            with col1:
                info["programmer_brand"] = st.text_input('What Programmer (Brand) do you want us to use?', placeholder="FlashRunner 2.0, FRCube, Phyton, Algocraft, Segger, etc.")
                info['logistic_data'] = st.radio(OPTIONS[0], YES_NO, index=1, horizontal=True)
            with col2:
                ...
                info["items_for_logistic_data"] = st.text_input(OPTIONS[1], placeholder="Part Number, Serial Number, Security Key etc.")

def _render_test_specifications_section(info):
    with st.container(border=True):
        subtitle_h3("Test Specifications")
        info['test_spec'] = st.radio(OPTIONS[3], YES_NO, index=1, horizontal=True)
        info['fixture_sow'] = st.radio(OPTIONS[4], YES_NO, index=1, horizontal=True)
        info['config_file'] = st.radio(OPTIONS[2], YES_NO, index=1, horizontal=True)

def _render_panel_section(info):
    with st.container(border=True):
...
def _render_additional_requirements_section(info):
    with st.container(border=True):
        ...
        with st.container(border=True):
            ...
            col1, col2 = st.columns(2)
            with col1:
                info['color_test'] = st.radio('Color/intensity LED test required and preferred sensor?', YES_NO, index=1, horizontal=True)
            with col2:
                info['color_test_info'] = st.text_input('Specify Sensor', placeholder="Color Test, Feasa Led Analyzer/Optimistic.")
...
def create_ict_specific_sections(info):
    ...
    """
    _render_feature_fixture_section(info)
    _render_flash_programming_section(info)
    _render_test_specifications_section(info)
    _render_panel_section(info)
    _render_additional_requirements_section(info)
